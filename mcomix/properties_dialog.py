...
class _PropertiesDialog(gtk.Dialog):
    ...
    def __init__(self, window):
        ...
        self._window.draw_image += self._on_page_change
        self._window.filehandler.file_opened += self._on_archive_change
        self._window.filehandler.close_file += self._on_archive_change
    ...
    def _on_archive_change(self, *args, **kwargs):
        self._update_archive_page()
    ...
    def _update_page_image(self, page, page_number=None):
        if not self._window.imagehandler.page_is_available(page_number):
            return
        thumb = self._window.imagehandler.get_thumbnail(page_number, width=200, height=128)
